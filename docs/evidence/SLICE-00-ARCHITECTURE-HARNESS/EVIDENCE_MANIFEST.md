...
CAPTURE TOOL: Microsoft Edge headless via Chrome DevTools Protocol, plus FFmpeg for WebM assembly  
PLAYWRIGHT STATUS: NOT AVAILABLE without installation; see `diagnostics/playwright-status.json`
...
| `diagnostics/playwright-status.json` | Playwright automation status and why it was not used. | MEASURED | No install attempted. |
