...
| Playwright path not green. | Kept `playwright-status.json` as NOT AVAILABLE and documented Edge CDP as the passing browser QA route. No Playwright pass is claimed. | `diagnostics/playwright-status.json`, `diagnostics/browser-qa-status.json` |
...
- Playwright remains unavailable without installation.
